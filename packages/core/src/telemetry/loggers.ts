...
import { NebiusLogger } from './nebius-logger/nebius-logger.js';
...
export function logCliConfiguration(
  ...
  NebiusLogger.getInstance(config)?.logStartSessionEvent(event);
...
export function logUserPrompt(config: Config, event: UserPromptEvent): void {
  NebiusLogger.getInstance(config)?.logNewPromptEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logToolCall(config: Config, event: ToolCallEvent): void {
  ...
  NebiusLogger.getInstance(config)?.logToolCallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiRequest(config: Config, event: ApiRequestEvent): void {
  NebiusLogger.getInstance(config)?.logApiRequestEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logFlashFallback(
  ...
  NebiusLogger.getInstance(config)?.logFlashFallbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiError(config: Config, event: ApiErrorEvent): void {
  ...
  uiTelemetryService.addEvent(uiEvent);
  NebiusLogger.getInstance(config)?.logApiErrorEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiResponse(config: Config, event: ApiResponseEvent): void {
  ...
  uiTelemetryService.addEvent(uiEvent);
  NebiusLogger.getInstance(config)?.logApiResponseEvent(event);
  if (!isTelemetrySdkInitialized()) return;
  const attributes: LogAttributes = {
    ...getCommonAttributes(config),
    ...event,
...
export function logLoopDetected(
  ...
  NebiusLogger.getInstance(config)?.logLoopDetectedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logNextSpeakerCheck(
  ...
  NebiusLogger.getInstance(config)?.logNextSpeakerCheck(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logSlashCommand(
  ...
  NebiusLogger.getInstance(config)?.logSlashCommandEvent(event);
  if (!isTelemetrySdkInitialized()) return;
