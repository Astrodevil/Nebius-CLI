...
import { NebiusContentGenerator } from './nebiusContentGenerator.js';
...
describe('NebiusContentGenerator', () => {
  let mockNebiusClient: INebiusOAuth2Client;
  let qwenContentGenerator: NebiusContentGenerator;
  let mockConfig: Config;

  const mockCredentials: NebiusCredentials = {
  ...
  beforeEach(() => {
    ...
    // Create NebiusContentGenerator instance
    qwenContentGenerator = new NebiusContentGenerator(
      mockNebiusClient,
      'qwen-turbo',
      mockConfig,
    );
  ...
  describe('Core Content Generation Methods', () => {
    it('should generate content with valid token', async () => {
      vi.mocked(mockNebiusClient.getAccessToken).mockResolvedValue({
        token: 'valid-token',
      });
      vi.mocked(mockNebiusClient.getCredentials).mockReturnValue(
        mockCredentials,
      );
      ...
      expect(mockNebiusClient.getAccessToken).toHaveBeenCalled();
    });
  ...
  describe('Token Management and Refresh Logic', () => {
    it('should refresh token on auth error and retry', async () => {
      ...
      vi.mocked(mockNebiusClient.getAccessToken).mockRejectedValueOnce(
        authError,
      );

      // Refresh succeeds
      vi.mocked(mockNebiusClient.refreshAccessToken).mockResolvedValue({
      ...
      expect(result.text).toBe('Generated content');
      expect(mockNebiusClient.refreshAccessToken).toHaveBeenCalled();
    });

    it('should handle token refresh failure', async () => {
      vi.mocked(mockNebiusClient.getAccessToken).mockRejectedValue(
        new Error('Token expired'),
      );
      vi.mocked(mockNebiusClient.refreshAccessToken).mockRejectedValue(
      ...
      await expect(
        ...
        'Failed to obtain valid Nebius access token. Please re-authenticate.',
      );
    });
    ...
    it('should update endpoint when token is refreshed', async () => {
      ...
      expect(mockNebiusClient.getCredentials).toHaveBeenCalled();
    });
  });
  ...
  describe('Error Handling and Retry Logic', () => {
    ...
    it('should not retry non-authentication errors', async () => {
      ...
      expect(mockNebiusClient.refreshAccessToken).not.toHaveBeenCalled();
    ...
    it('should handle error response from token refresh', async () => {
      ...
      await expect(
        qwenContentGenerator.generateContent(request, 'test-prompt-id'),
      ).rejects.toThrow('Failed to obtain valid Nebius access token');
    });
  });
  ...
  describe('Token State Management', () => {
    ...
    it('should handle concurrent token refresh requests', async () => {
      ...
      vi.mocked(mockNebiusClient.refreshAccessToken).mockImplementation(
        async () => {
          refreshCallCount++;
