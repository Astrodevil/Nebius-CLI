...
describe('NebiusOAuth2Client', () => {
  let client: NebiusOAuth2Client;
  ...
  beforeEach(() => {
    // Setup mock config
    _mockConfig = {
      getNebiusClientId: vi.fn().mockReturnValue('test-client-id'),
      isBrowserLaunchSuppressed: vi.fn().mockReturnValue(false),
      getProxy: vi.fn().mockReturnValue(undefined),
    } as unknown as Config;

    // Create client instance
    client = new NebiusOAuth2Client({ proxy: undefined });
  ...
  describe('refreshAccessToken', () => {
    ...
    it('should cache credentials after successful refresh', async () => {
      ...
      // Verify that cacheNebiusCredentials was called by checking if writeFile was called
...
describe('getNebiusOAuthClient', () => {
  ...
  it('should create client with proxy configuration', async () => {
    ...
    try {
      await import('./nebiusOAuth2.js').then((module) =>
        module.getNebiusOAuthClient(mockConfig),
      );
    } catch {
  ...
  it('should load cached credentials if available', async () => {
    ...
    const client = await import('./nebiusOAuth2.js').then((module) =>
      module.getNebiusOAuthClient(mockConfig),
    );
  ...
  it('should handle cached credentials refresh failure', async () => {
    ...
    const mockCredentials = {
      ...
      token_type: 'Bearer',
      expiry_date: Date.now() + 3600000, // Valid expiry time so loadCachedNebiusCredentials returns true
    };
    ...
    await expect(
      import('./nebiusOAuth2.js').then((module) =>
        module.getNebiusOAuthClient(mockConfig),
      ),
    ).rejects.toThrow('Cached Nebius credentials are invalid');
  });
});

describe('clearNebiusCredentials', () => {
  it('should successfully clear credentials file', async () => {
    const { promises: fs } = await import('node:fs');
    const { clearNebiusCredentials } = await import('./nebiusOAuth2.js');

    vi.mocked(fs.unlink).mockResolvedValue(undefined);

    await expect(clearNebiusCredentials()).resolves.not.toThrow();
...
describe('NebiusOAuth2Client - Additional Error Scenarios', () => {
  let client: NebiusOAuth2Client;
  let originalFetch: typeof global.fetch;
...
describe('getNebiusOAuthClient - Enhanced Error Scenarios', () => {
  let mockConfig: Config;
  let originalFetch: typeof global.fetch;
  ...
  it('should handle generic refresh token errors', async () => {
    ...
    await expect(
      ...
      'Nebius token refresh failed: Token refresh failed: 500 Internal Server Error',
    );
  });
  ...
  it('should handle different authentication failure reasons - timeout', async () => {
    ...
    ).rejects.toThrow('Nebius OAuth authentication timed out');
  });
  ...
  it('should handle authentication failure reason - error', async () => {
    ...
    ).rejects.toThrow('Nebius OAuth authentication failed');
  });
});

describe('authWithNebiusDeviceFlow - Comprehensive Testing', () => {
  let mockConfig: Config;
  let originalFetch: typeof global.fetch;
  let _client: NebiusOAuth2Client;
  ...
  beforeEach(() => {
    ...
    _client = new NebiusOAuth2Client({ proxy: undefined });
    originalFetch = global.fetch;
    global.fetch = vi.fn();
...
describe('Event Emitter Integration', () => {
  it('should export qwenOAuth2Events as EventEmitter', async () => {
    const { qwenOAuth2Events } = await import('./nebiusOAuth2.js');
    expect(qwenOAuth2Events).toBeInstanceOf(EventEmitter);
  });

  it('should define correct event enum values', async () => {
    const { NebiusOAuth2Event } = await import('./nebiusOAuth2.js');
    expect(NebiusOAuth2Event.AuthUri).toBe('auth-uri');
    expect(NebiusOAuth2Event.AuthProgress).toBe('auth-progress');
    expect(NebiusOAuth2Event.AuthCancel).toBe('auth-cancel');
  });
});
