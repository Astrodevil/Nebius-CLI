...
import {
  MemoryTool,
  setGeminiMdFilename,
  NEBIUS_CONFIG_DIR as NEBIUS_DIR,
...
import { NebiusLogger } from '../telemetry/nebius-logger/nebius-logger.js';
...
export class Config {
  ...
  constructor(params: ConfigParameters) {
    ...
    this.gitCoAuthor = {
      ...
      name: params.gitCoAuthor?.name ?? 'Nebius-Coder',
      email: params.gitCoAuthor?.email ?? 'nebius-coder@alibabacloud.com',
    };
    ...
    // Web search
    // Prefer explicit parameter; otherwise fall back to environment variable.
    this.tavilyApiKey = params.tavilyApiKey ?? process.env.TAVILY_API_KEY;
    this.linkupApiKey = params.linkupApiKey ?? process.env.LINKUP_API_KEY;
    // If no API key present, effectively disabled; keep flag true so enabling is a no-op until key is set
    ...
    if (this.getUsageStatisticsEnabled()) {
      NebiusLogger.getInstance(this)?.logStartSessionEvent(
  ...
  async refreshAuth(authMethod: AuthType) {
    ...
    // Small delay to ensure environment variables are properly set
    // This fixes a race condition where API keys set just before refreshAuth
    // might not be available when the content generator is created
  ...
  getGeminiDir(): string {
    return path.join(this.targetDir, NEBIUS_DIR);
  }
  ...
  getLinkupApiKey(): string | undefined {
    return this.linkupApiKey;
  }

  setLinkupApiKey(apiKey: string): void {
    // Cast to mutable to allow runtime updates
    (this as any).linkupApiKey = apiKey;
    // Also set in environment for child processes
    process.env.LINKUP_API_KEY = apiKey;
  }

  // Legacy Tavily support (deprecated)
  ...
  async createToolRegistry(): Promise<ToolRegistry> {
    ...
    // Conditionally register web search tool only if Linkup API key is set
    if (this.getLinkupApiKey()) {
