...
import { NEBIUS_DIR } from '../utils/paths.js';
...
export function getCoreSystemPrompt(
  ...
  // default path is .nebius/system.md but can be modified via custom path in GEMINI_SYSTEM_MD
  let systemMdEnabled = false;
  let systemMdPath = path.resolve(path.join(os.homedir(), NEBIUS_DIR, 'system.md'));
  ...
  if (config?.systemPromptMappings) {
    const currentModel = process.env.NEBIUS_MODEL || '';
    const currentBaseUrl = process.env.NEBIUS_BASE_URL || '';
  ...
  const basePrompt = systemMdEnabled
    ...
    : `
You are Nebius CLI, an interactive CLI agent specializing in software engineering tasks. Your primary goal is to help users safely and efficiently, adhering strictly to the following instructions and utilizing your available tools.
