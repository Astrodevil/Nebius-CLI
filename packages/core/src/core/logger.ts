...
export class Logger {
  private nebiusDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.nebiusDir = getProjectTempDir(process.cwd());
    this.logFilePath = path.join(this.nebiusDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.nebiusDir, { recursive: true });
  ...
  _checkpointPath(tag: string): string {
    ...
    return path.join(this.nebiusDir, `checkpoint-${sanitizedTag}.json`);
  }
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.nebiusDir) {
      console.error(
