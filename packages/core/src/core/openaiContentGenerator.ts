...
import { NEBIUS_BASE_URL } from '../config/models.js';
...
export class OpenAIContentGenerator implements ContentGenerator {
  ...
  constructor(apiKey: string, model: string, config: Config) {
    this.model = model;
    this.config = config;
    const baseURL = process.env.NEBIUS_BASE_URL || NEBIUS_BASE_URL;
    ...
    const userAgent = `NebiusCode/${version} (${process.platform}; ${process.arch})`;
