...
// Note: Using GEMINI_DIR instead of NEBIUS_DIR to match compiled JavaScript
...
describe('Core System Prompt (prompts.ts)', () => {
  ...
  it('should return the base prompt when no userMemory is provided', () => {
    ...
    expect(prompt).toContain('You are Nebius CLI, an interactive CLI agent'); // Check for core content
  ...
  it('should return the base prompt when userMemory is empty string', () => {
    ...
    const prompt = getCoreSystemPrompt('');
    expect(prompt).not.toContain('---\n\n');
    expect(prompt).toContain('You are Nebius CLI, an interactive CLI agent');
  ...
  it('should append userMemory with separator when provided', () => {
    ...
    expect(prompt).toContain('You are Nebius CLI, an interactive CLI agent'); // Ensure base prompt follows
  ...
  describe('GEMINI_SYSTEM_MD environment variable', () => {
    ...
    it('should read from default path when GEMINI_SYSTEM_MD is "true"', () => {
      const homeDir = '/Users/akshitgoyal';
...
describe('URL matching with trailing slash compatibility', () => {
  it('should match URLs with and without trailing slash', () => {
    ...
    process.env = {
      ...originalEnv,
      NEBIUS_BASE_URL: 'https://api.openai.com',
      NEBIUS_MODEL: 'gpt-3.5-turbo',
    };
  ...
  it('should not match when URLs are different', () => {
    ...
    process.env = {
      ...originalEnv,
      NEBIUS_BASE_URL: 'https://api.different.com',
      NEBIUS_MODEL: 'gpt-4',
    };
