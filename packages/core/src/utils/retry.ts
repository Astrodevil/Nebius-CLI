...
import {
  ...
  isNebiusQuotaExceededError,
  isNebiusThrottlingError,
...
export async function retryWithBackoff<T>(
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      // Check for Nebius OAuth quota exceeded error - throw immediately without retry
      if (
        authType === AuthType.QWEN_OAUTH &&
        isNebiusQuotaExceededError(error)
      ) {
        throw new Error(
          `Nebius API quota exceeded: Your Nebius API quota has been exhausted. Please wait for your quota to reset.`,
        );
      }

      // Track consecutive 429 errors, but handle Nebius throttling differently
      if (errorStatus === 429) {
        // For Nebius throttling errors, we still want to track them for exponential backoff
        // but not for quota fallback logic (since Nebius doesn't have model fallback)
        if (
          authType === AuthType.QWEN_OAUTH &&
          isNebiusThrottlingError(error)
        ) {
