...
export const NEBIUS_DIR = '.nebius';
// Alias for backward compatibility with compiled JS
export const GEMINI_DIR = NEBIUS_DIR;
...
export function getProjectTempDir(projectRoot: string): string {
  ...
  return path.join(os.homedir(), NEBIUS_DIR, TMP_DIR_NAME, hash);
}
...
export function getUserCommandsDir(): string {
  return path.join(os.homedir(), NEBIUS_DIR, COMMANDS_DIR_NAME);
}
...
export function getProjectCommandsDir(projectRoot: string): string {
  return path.join(projectRoot, NEBIUS_DIR, COMMANDS_DIR_NAME);
}
