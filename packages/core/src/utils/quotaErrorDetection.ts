...
export function isNebiusQuotaExceededError(error: unknown): boolean {
  // Check for Nebius insufficient quota errors (should not retry)
  const checkMessage = (message: string): boolean => {
...
export function isNebiusThrottlingError(error: unknown): boolean {
  // Check for Nebius throttling errors (should retry)
  const checkMessage = (message: string): boolean => {
    const lowerMessage = message.toLowerCase();
    return (
