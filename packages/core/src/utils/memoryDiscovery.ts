  ...
  for (const geminiMdFilename of geminiMdFilenames) {
    ...
    if (dir) {
      ...
      while (currentDir && currentDir !== path.dirname(currentDir)) {
        if (currentDir === path.join(resolvedHome, NEBIUS_CONFIG_DIR)) {
          break;
        }
...
/**
 * Loads hierarchical NEBIUS.md files and concatenates their content.
...
export async function loadServerHierarchicalMemory(
  ...
  if (filePaths.length === 0) {
    if (debugMode) logger.debug('No NEBIUS.md files found in hierarchy.');
