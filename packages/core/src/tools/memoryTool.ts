...
import { tildeifyPath, NEBIUS_DIR } from '../utils/paths.js';
  ...
  parameters: {
    ...
    properties: {
      ...
      scope: {
        type: Type.STRING,
        description:
          'Where to save the memory: "global" saves to user-level ~/.nebius/NEBIUS.md (shared across all projects), "project" saves to current project\'s NEBIUS.md (project-specific). If not specified, will prompt user to choose.',
        enum: ['global', 'project'],
      },
    },
    required: ['fact'],
...
- \`scope\` (string, optional): Where to save the memory:
  - "global": Saves to user-level ~/.nebius/NEBIUS.md (shared across all projects)
  - "project": Saves to current project's NEBIUS.md (project-specific)
  - If not specified, the tool will ask the user where they want to save the memory.
`;

// Use the same constant from paths.js for consistency
export const NEBIUS_CONFIG_DIR = NEBIUS_DIR;
export const DEFAULT_CONTEXT_FILENAME = 'NEBIUS.md';
export const MEMORY_SECTION_HEADER = '## Nebius Added Memories';

// This variable will hold the currently configured filename for NEBIUS.md context files.
...
function getGlobalMemoryFilePath(): string {
  return path.join(homedir(), NEBIUS_CONFIG_DIR, getCurrentGeminiMdFilename());
}
  ...
  implements ModifiableTool<SaveMemoryParams>
{
  ...
  async execute(
    ...
    // Be robust: if scope is missing here (e.g., direct server call), default to global
    const scope = params.scope || 'global';
    const memoryFilePath = getMemoryFilePath(scope);
