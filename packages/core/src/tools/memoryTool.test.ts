...
const MEMORY_SECTION_HEADER = '## Nebius Added Memories';
  ...
  describe('shouldConfirmExecute', () => {
    ...
    it('should return confirmation details when memory file is not allowlisted', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join('~', '.nebius', 'NEBIUS.md');
        expect(result.title).toBe(
          `Confirm Memory Save: ${expectedPath} (global)`,
        );
        expect(result.fileName).toContain(path.join('mock', 'home', '.nebius'));
        expect(result.fileName).toContain('NEBIUS.md');
        expect(result.fileDiff).toContain('Index: NEBIUS.md');
        expect(result.fileDiff).toContain('+## Nebius Added Memories');
        expect(result.fileDiff).toContain('+- Test fact');
        expect(result.originalContent).toBe('');
        expect(result.newContent).toContain('## Nebius Added Memories');
