...
`;
export const GEMINI_CONFIG_DIR = '.nebius';
...
export class MemoryTool extends BaseTool {
   ...
   */
  async readMemoryFileContent(scope = 'global') {
    try {
  ...
  async execute(params, _signal) {
    ...
    if (!params.scope) {
      const errorMessage =
        'Please specify where to save this memory. Use scope parameter: "global" for user-level (~/.nebius/NEBIUS.md) or "project" for current project (./NEBIUS.md).';
      return {
        llmContent: JSON.stringify({ success: false, error: errorMessage }),
  ...
  getModifyContext(_abortSignal) {
    return {
      getFilePath: (params) => getMemoryFilePath(params.scope || 'global'),
