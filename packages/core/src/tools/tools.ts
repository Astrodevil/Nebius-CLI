...
export interface ToolResult {
  ...
  /**
   * If this property is present, the tool requires an API key that is missing.
   * The UI should prompt the user to add the API key.
   */
  requiresApiKey?: {
    provider: string; // e.g., 'linkup', 'tavily'
    keyName: string; // e.g., 'LINKUP_API_KEY'
    description: string; // User-friendly description
    signupUrl?: string; // URL where user can sign up for the service
  };
}
