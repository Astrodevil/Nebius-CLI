...
export class ToolRegistry {
  ...
  getFunctionDeclarations() {
    const declarations = [];
    const webSearchDisabled = !this.config.getWebSearchEnabled();
    this.tools.forEach((tool) => {
      // If web search disabled, skip exposing the web_search function to the model
      if (webSearchDisabled && tool.name === 'web_search') {
        return;
      }
