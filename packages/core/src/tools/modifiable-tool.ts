...
function createTempFilesForModify(
  ...
  const diffDir = path.join(tempDir, 'nebius-code-tool-modify-diffs');
  ...
  const tempOldPath = path.join(
    diffDir,
    `nebius-code-modify-${fileName}-old-${timestamp}${ext}`,
  );
  const tempNewPath = path.join(
    diffDir,
    `nebius-code-modify-${fileName}-new-${timestamp}${ext}`,
  );
