...
/**
 * A tool to perform web searches using Linkup API.
 */
...
> {
  ...
  constructor(private readonly config: Config) {
    super(
      WebSearchTool.Name,
      'LinkupSearch',
      'Performs a web search using the Linkup API and returns a concise answer with sources. Requires the LINKUP_API_KEY environment variable.',
      Icon.Globe,
      {
        type: Type.OBJECT,
        properties: {
  ...
  async execute(
    ...
    const apiKey = this.config.getLinkupApiKey() || process.env.LINKUP_API_KEY;
    if (!apiKey) {
      return {
        llmContent:
          'Web search requires a Linkup API key. Please use the /websearch command to configure your API key, or set LINKUP_API_KEY environment variable.',
        returnDisplay:
          'Linkup API key required for web search. Use /websearch to configure.',
        requiresApiKey: {
          provider: 'linkup',
          keyName: 'LINKUP_API_KEY',
          description: 'Linkup API key for web search functionality',
          signupUrl: 'https://linkup.so'
        }
      };
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Linkup API error: ${response.status} ${response.statusText}${text ? ` - ${text}` : ''}`,
        );
      }
