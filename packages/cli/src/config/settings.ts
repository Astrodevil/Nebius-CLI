...
// Temporarily define locally to avoid import issues
...
export const SETTINGS_DIRECTORY_NAME = '.nebius';
...
export function getSystemSettingsPath(): string {
  ...
  if (platform() === 'darwin') {
    return '/Library/Application Support/NebiusCode/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\nebius-code\\settings.json';
  } else {
    return '/etc/nebius-code/settings.json';
  }
}
...
function findEnvFile(startDir: string): string | null {
  ...
  while (true) {
    ...
    // prefer nebius-specific .env under NEBIUS_DIR
    const nebiusEnvPath = path.join(currentDir, NEBIUS_DIR, '.env');
    if (fs.existsSync(nebiusEnvPath)) {
      return nebiusEnvPath;
    }
    ...
    if (parentDir === currentDir || !parentDir) {
      // check .env under home as fallback, again preferring nebius-specific .env
      const homeNebiusEnvPath = path.join(homedir(), NEBIUS_DIR, '.env');
      if (fs.existsSync(homeNebiusEnvPath)) {
        return homeNebiusEnvPath;
      }
...
export function loadEnvironment(settings?: Settings): void {
  ...
  const envFilePath = findEnvFile(cwd);
  ...
  if (!resolvedSettings) {
    const workspaceSettingsPath = getWorkspaceSettingsPath(cwd);
    try {
  ...
  if (envFilePath) {
    ...
    try {
      ...
      const isProjectEnvFile = !envFilePath.includes(NEBIUS_DIR);
