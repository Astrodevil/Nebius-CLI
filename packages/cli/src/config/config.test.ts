...
import * as ServerConfig from '@nebius-code/nebius-code-core';
...
describe('Hierarchical Memory Loading (config.ts) - Placeholder Suite', () => {
    ...
    const extensions: Extension[] = [
      {
        ...
        contextFiles: ['/path/to/ext1/NEBIUS.md'],
      },
      {
        config: {
          name: 'ext2',
  ...
  it('should correctly use mocked homedir for global path', async () => {
    const MOCK_GEMINI_DIR_LOCAL = path.join('/mock/home/user', '.nebius');
    const MOCK_GLOBAL_PATH_LOCAL = path.join(MOCK_GEMINI_DIR_LOCAL, 'NEBIUS.md');
    mockFs({
    ...
    expect(config.getSystemPromptMappings()).toEqual([
      {
        ...
        modelNames: ['openai/gpt-oss-120b'],
        template:
...
describe('loadCliConfig model selection', () => {
  ...
  it('uses the default gemini model if nothing is set', async () => {
    ...
    expect(config.getModel()).toBe('Qwen/Qwen3-32B-fast');
  });
