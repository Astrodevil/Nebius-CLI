...
export async function parseArguments(): Promise<CliArgs> {
  const yargsInstance = yargs(hideBin(process.argv))
    ...
    .usage(
      '$0 [options]',
      'Nebius CLI - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
    .option('model', {
      alias: 'm',
      type: 'string',
    ...
    .option('telemetry-target', {
      ...
      choices: ['local'],
      description:
        'Set the telemetry target (local). Overrides settings files.',
    })
    ...
    .option('load-memory-from-include-directories', {
      type: 'boolean',
      description:
        'If true, when refreshing memory, NEBIUS.md files should be loaded from all directories that are added. If false, NEBIUS.md files should only be loaded from the primary working directory.',
      default: false,
    })
...
export async function loadCliConfig(
  ...
  if (argv.openaiApiKey) {
    process.env.NEBIUS_API_KEY = argv.openaiApiKey;
  }

  // Handle OpenAI base URL from command line
  if (argv.openaiBaseUrl) {
    process.env.NEBIUS_BASE_URL = argv.openaiBaseUrl;
  }
  ...
  return new Config({
    ...
    systemPromptMappings: settings.systemPromptMappings ?? [
      {
        baseUrls: ['https://api.studio.nebius.com/v1/'],
        modelNames: [
          existingModel
            ? existingModel
            : 'openai/gpt-oss-120b',
