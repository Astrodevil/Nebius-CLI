...
describe('loadExtensions', () => {
  ...
  beforeEach(() => {
    tempWorkspaceDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'nebius-code-test-workspace-'),
    );
    tempHomeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'nebius-code-test-home-'),
    );
  ...
  it('should load context file path when NEBIUS.md is present', () => {
    const workspaceExtensionsDir = path.join(
      tempWorkspaceDir,
      EXTENSIONS_DIRECTORY_NAME,
    );
    ...
    expect(ext1?.contextFiles).toEqual([
      path.join(workspaceExtensionsDir, 'ext1', 'NEBIUS.md'),
    ]);
...
function createExtension(
  ...
  if (addContextFile) {
    fs.writeFileSync(path.join(extDir, 'NEBIUS.md'), 'context');
  }
