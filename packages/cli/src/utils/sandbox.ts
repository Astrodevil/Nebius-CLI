...
import { Config, SandboxConfig } from '@nebius-code/nebius-code-core';
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'nebius-code-sandbox';
const SANDBOX_NETWORK_NAME = 'nebius-code-sandbox';
const SANDBOX_PROXY_NAME = 'nebius-code-sandbox-proxy';
...
function entrypoint(workdir: string): string[] {
  ...
  const cliCmd =
    process.env.NODE_ENV === 'development'
      ...
      : process.env.DEBUG
        ? `node --inspect-brk=0.0.0.0:${process.env.DEBUG_PORT || '9229'} $(which nebius)`
        : 'nebius';
...
export async function start_sandbox(
  ...
  if (config.command === 'sandbox-exec') {
    ...
    // start and set up proxy if NEBIUS_SANDBOX_PROXY_COMMAND is set
    const proxyCommand = process.env.NEBIUS_SANDBOX_PROXY_COMMAND;
  ...
  // determine full path for nebius-cli to distinguish linked vs installed setting
  ...
  // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under nebius-cli repo
  //
  // note this can only be done with binary linked from nebius-cli repo
  if (process.env.BUILD_SANDBOX) {
    if (!gcPath.includes('gemini-cli/packages/')) {
      console.error(
        'ERROR: cannot build sandbox using installed gemini binary; ' +
          'run `npm link ./packages/cli` under nebius-cli repo to switch to linked binary.',
      );
      process.exit(1);
    } else {
      ...
      execSync(
        `cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`,
        {
          ...
          env: {
            ...process.env,
            NEBIUS_SANDBOX: config.command, // in case sandbox is enabled via flags (see config.ts under cli package)
          },
        },
      );
    }
  ...
  if (!(await ensureSandboxImageIsPresent(config.command, image))) {
    const remedy =
      image === LOCAL_DEV_SANDBOX_IMAGE_NAME
        ? 'Try running `npm run build:all` or `npm run build:sandbox` under the nebius-cli repo to build it locally, or check the image name and your network connection.'
        : 'Please check the image name, your network connection, or notify nebius-cli-dev@google.com if the issue persists.';
    console.error(
  ...
  // NEBIUS_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
  const proxyCommand = process.env.NEBIUS_SANDBOX_PROXY_COMMAND;
  ...
  // copy NEBIUS_API_KEY(s)
  if (process.env.NEBIUS_API_KEY) {
    args.push('--env', `NEBIUS_API_KEY=${process.env.NEBIUS_API_KEY}`);
  }
  if (process.env.GOOGLE_API_KEY) {
    args.push('--env', `GOOGLE_API_KEY=${process.env.GOOGLE_API_KEY}`);
  }

  // copy NEBIUS_API_KEY and related env vars for Nebius
  if (process.env.NEBIUS_API_KEY) {
    args.push('--env', `NEBIUS_API_KEY=${process.env.NEBIUS_API_KEY}`);
  }
  ...
  if (process.env.NEBIUS_BASE_URL) {
    args.push('--env', `NEBIUS_BASE_URL=${process.env.NEBIUS_BASE_URL}`);
  }
  if (process.env.NEBIUS_MODEL) {
    args.push('--env', `NEBIUS_MODEL=${process.env.NEBIUS_MODEL}`);
  }
  ...
  if (process.env.NEBIUS_CLI_INTEGRATION_TEST === 'true') {
    args.push('--user', 'root');
    userFlag = '--user root';
