...
describe('validateNonInterActiveAuth', () => {
  ...
  beforeEach(() => {
    ...
    originalEnvOpenAiApiKey = process.env.NEBIUS_API_KEY;
  ...
  afterEach(() => {
    ...
    if (originalEnvOpenAiApiKey !== undefined) {
      process.env.NEBIUS_API_KEY = originalEnvOpenAiApiKey;
    } else {
      delete process.env.NEBIUS_API_KEY;
    }
    vi.restoreAllMocks();
  ...
  it('uses USE_OPENAI if NEBIUS_API_KEY is set', async () => {
    process.env.NEBIUS_API_KEY = 'fake-openai-key';
    const nonInteractiveConfig: NonInteractiveConfig = {
      refreshAuth: refreshAuthMock,
    };
    await validateNonInteractiveAuth(
