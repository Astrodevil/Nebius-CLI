...
import { NebiusLight } from './nebius-light.js';
import { NebiusDark } from './nebius-dark.js';
import {
  Theme,
  ThemeType,
  CustomTheme,
...
export const DEFAULT_THEME: Theme = NebiusDark;
...
class ThemeManager {
  ...
  getAvailableThemes(): ThemeDisplay[] {
    ...
    // Separate Nebius themes
    const qwenThemes = builtInThemes.filter(
      (theme) =>
        theme.name === NebiusLight.name || theme.name === NebiusDark.name,
    );
    ...
    // Combine Nebius themes first, then sorted others
