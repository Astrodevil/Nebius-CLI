...
// Mock @nebius-code/nebius-code-core and its Config class
vi.mock('@nebius-code/nebius-code-core', async (importOriginal) => {
  const actualCore =
    await importOriginal<typeof import('@nebius-code/nebius-code-core')>();
  const ConfigClassMock = vi
    ...
    .mockImplementation((optionsPassedToConstructor) => {
      ...
      return {
        ...
        getAllGeminiMdFilenames: vi.fn(() => ['NEBIUS.md']),
...
const { isGitRepository: mockedIsGitRepository } = vi.mocked(
  await import('@nebius-code/nebius-code-core'),
);
...
describe('App UI', () => {
  ...
  describe('handleAutoUpdate', () => {
    ...
    it('should not start the update process when running from git', async () => {
      ...
      const info: UpdateObject = {
        ...
        message: 'Nebius CLI update available!',
      };
  ...
  it('should display active file and other context', async () => {
    ...
    mockConfig.getAllGeminiMdFilenames.mockReturnValue(['NEBIUS.md']);
    ...
    await Promise.resolve();
    expect(lastFrame()).toContain(
      'Using: 1 open file (ctrl+e to view) | 1 NEBIUS.md file',
    );
  });

  it('should display default "NEBIUS.md" in footer when contextFileName is not set and count is 1', async () => {
    mockConfig.getGeminiMdFileCount.mockReturnValue(1);
    mockConfig.getAllGeminiMdFilenames.mockReturnValue(['NEBIUS.md']);
    ...
    expect(lastFrame()).toContain('Using: 1 NEBIUS.md file');
  });

  it('should display default "NEBIUS.md" with plural when contextFileName is not set and count is > 1', async () => {
    ...
    currentUnmount = unmount;
    await Promise.resolve();
    expect(lastFrame()).toContain('Using: 2 NEBIUS.md files');
  });
  ...
  it('should display NEBIUS.md and MCP server count when both are present', async () => {
    mockConfig.getGeminiMdFileCount.mockReturnValue(2);
    mockConfig.getAllGeminiMdFilenames.mockReturnValue([
      'NEBIUS.md',
      'NEBIUS.md',
  ...
  it('should display only MCP server count when NEBIUS.md count is 0', async () => {
    mockConfig.getGeminiMdFileCount.mockReturnValue(0);
    mockConfig.getAllGeminiMdFilenames.mockReturnValue([]);
    mockConfig.getMcpServers.mockReturnValue({
      server1: {} as MCPServerConfig,
