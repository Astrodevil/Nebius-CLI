...
describe('bugCommand', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('SANDBOX', 'nebius-test');
  });
  ...
  it('should generate the default mailto support link', async () => {
    const mockContext = createMockCommandContext({
      services: {
        config: {
          getModel: () => 'Qwen/Qwen3-32B-fast',
...
* **Model Version:** Qwen/Qwen3-32B-fast
* **Memory Usage:** 100 MB
`;
    const expectedUrl =
      'mailto:ai-studio-support@nebius.ai?subject=' +
      encodeURIComponent('A test bug') +
      '&body=' +
