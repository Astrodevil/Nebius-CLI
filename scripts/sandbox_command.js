...
let geminiSandbox = process.env.NEBIUS_SANDBOX;

if (!geminiSandbox) {
  const userSettingsFile = join(os.homedir(), '.nebius', 'settings.json');
...
if (!geminiSandbox) {
  ...
  while (true) {
    const geminiEnv = join(currentDir, '.nebius', '.env');
  ...
  geminiSandbox = process.env.NEBIUS_SANDBOX;
}
...
if (['1', 'true'].includes(geminiSandbox)) {
  if (commandExists('docker')) {
    ...
    console.error(
      'ERROR: install docker or podman or specify command in NEBIUS_SANDBOX',
    );
    process.exit(1);
  }
  ...
  if (commandExists(geminiSandbox)) {
    ...
    console.error(
      `ERROR: missing sandbox command '${geminiSandbox}' (from NEBIUS_SANDBOX)`,
    );
    process.exit(1);
  }
} else {
