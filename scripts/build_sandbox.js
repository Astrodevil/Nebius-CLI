...
if (!baseImage?.length) {
  console.warn(
    'No default image tag specified in nebius-cli/packages/cli/package.json',
  );
}
...
console.log('packing @nebius-code/nebius-code ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'nebius-code-*.tgz'), { force: true });
execSync(
  `npm pack -w @nebius-code/nebius-code --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing @nebius-code/nebius-code-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'nebius-code-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w @nebius-code/nebius-code-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `nebius-code-nebius-code-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(
    corePackageDir,
    'dist',
    `nebius-code-nebius-code-core-${packageVersion}.tgz`,
  ),
  0o755,
);
...
function buildImage(imageName, dockerfile) {
  ...
  const imageTag =
    process.env.NEBIUS_SANDBOX_IMAGE_TAG || imageName.split(':')[1];
