...
const USER_GEMINI_DIR = path.join(os.homedir(), '.nebius');
// Project-level .gemini directory in the workspace
const WORKSPACE_GEMINI_DIR = path.join(projectRoot, '.nebius');

// Telemetry artifacts are stored in a hashed directory under the user's ~/.nebius/tmp
...
export function getJson(url) {
  const tmpFile = path.join(
    os.tmpdir(),
    `nebius-code-releases-${Date.now()}.json`,
  );
  try {
    execSync(
      `curl -sL -H "User-Agent: nebius-code-dev-script" -o "${tmpFile}" "${url}"`,
      { stdio: 'pipe' },
    );
...
export async function ensureBinary(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nebius-code-telemetry-'));
