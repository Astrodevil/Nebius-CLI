...
export class TestRig {
  ...
  getDefaultTimeout() {
    ...
    if (env.NEBIUS_SANDBOX) return 30000; // 30s in containers
  ...
  setup(testName, options = {}) {
    ...
    const geminiDir = join(this.testDir, '.nebius');
    mkdirSync(geminiDir, { recursive: true });
    // In sandbox mode, use an absolute path for telemetry inside the container
    // The container mounts the test directory at the same path as the host
    const telemetryPath =
      env.NEBIUS_SANDBOX && env.NEBIUS_SANDBOX !== 'false'
    ...
    const settings = {
      ...
      sandbox: env.NEBIUS_SANDBOX !== 'false' ? env.NEBIUS_SANDBOX : false,
  ...
  readToolLogs() {
    ...
    for (const jsonStr of jsonObjects) {
      try {
        ...
        if (
          logData.attributes &&
          logData.attributes['event.name'] === 'nebius-code.tool_call'
        ) {
