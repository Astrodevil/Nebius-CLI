...
This document describes the `web_fetch` tool for the Nebius CLI.
...
## How to use `web_fetch` with the Nebius CLI

To use `web_fetch` with the Nebius CLI, provide a URL and a prompt describing what you want to extract from that URL. The tool will ask for confirmation before fetching the URL. Once confirmed, the tool will fetch the content directly and process it using an AI model.
