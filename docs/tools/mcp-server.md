# MCP servers with the Nebius CLI

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with the Nebius CLI.

## What is an MCP server?

An MCP server is an application that exposes tools and resources to the Nebius CLI through the Model Context Protocol, allowing it to interact with external systems and data sources. MCP servers act as a bridge between the nebius model and your local environment or other services like APIs.

An MCP server enables the Nebius CLI to:
...
- **Access resources:** Read data from specific resources (though the Nebius CLI primarily focuses on tool execution).

With an MCP server, you can extend the Nebius CLI's capabilities to perform actions beyond its built-in features, such as interacting with databases, APIs, custom scripts, or specialized workflows.

## Core Integration Architecture

The Nebius CLI integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
4. **Sanitizes and validates** tool schemas for compatibility with the nebius API
...
The Nebius CLI supports three MCP transport types:
...
The Nebius CLI uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.

### Configure the MCP server in settings.json

You can configure MCP servers at the global level in the `~/.nebius/settings.json` file or in your project's root directory, create or open the `.nebius/settings.json` file. Within the file, add the `mcpServers` configuration block.
...
The Nebius CLI supports OAuth 2.0 authentication for remote MCP servers using SSE or HTTP transports. This enables secure access to MCP servers that require authentication.
...
- **Stored securely** in `~/.nebius/mcp-oauth-tokens.json`
...
When the Nebius CLI starts, it performs MCP server discovery through the following detailed process:
...
4. **Name sanitization:** Tool names are cleaned to meet nebius API requirements:
...
Tool parameter schemas undergo sanitization for nebius API compatibility:
...
When the nebius model decides to use an MCP tool, the following execution flow occurs:
...
Once discovered, MCP tools are available to the nebius model like built-in tools. The model will automatically:
...
- **Property stripping:** The system automatically removes certain schema properties (`$schema`, `additionalProperties`) for nebius API compatibility
- **Name sanitization:** Tool names are automatically sanitized to meet API requirements
- **Conflict resolution:** Tool name conflicts between servers are resolved through automatic prefixing

This comprehensive integration makes MCP servers a powerful way to extend the Nebius CLI's capabilities while maintaining security, reliability, and ease of use.
...
When the Nebius CLI receives this response, it will:
...
This enables you to build sophisticated tools that can provide rich, multi-modal context to the nebius model.

## MCP Prompts as Slash Commands

In addition to tools, MCP servers can expose predefined prompts that can be executed as slash commands within the Nebius CLI. This allows you to create shortcuts for common or complex queries that can be easily invoked by name.
...
```bash
/poem-writer --title="Nebius CLI" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "Nebius CLI" reverent
```

When you run this command, the Nebius CLI executes the `prompts/get` method on the MCP server with the provided arguments. The server is responsible for substituting the arguments into the prompt template and returning the final prompt text. The CLI then sends this prompt to the model for execution. This provides a convenient way to automate and share common workflows.
